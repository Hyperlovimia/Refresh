...
### 1.1 数据结构扩展
- [x] 在 `main.h` 中定义 `FAN_COUNT = 3` 常量
- [x] 在 `main.h` 中定义 `FanId` 枚举（FAN_0, FAN_1, FAN_2）
- [x] 在 `main.h` 中定义 `MultiFanState` 结构体（包含3个风扇状态的数组）

**验证**：编译通过

### 1.2 fan_control 模块重构
- [x] 修改 `fan_control.h` 接口定义：
...
### 2.1 命令协议扩展
- [x] 修改 `parse_remote_command()` 解析新格式：
  ```json
  {"fan_0":"HIGH", "fan_1":"LOW", "fan_2":"OFF"}
  ```
- [x] 修改 `s_remote_command` 为数组 `s_remote_command[3]`
- [x] 修改 `mqtt_get_remote_command()` 接口：
...
### 4.1 main.c 适配
- [x] 修改风扇状态变量为数组 `FanState fan_states[FAN_COUNT]`
- [x] 修改决策调用和状态上报调用
- [x] 修改OLED显示调用（如有需要）
...
### 5.1 编译验证
- [x] `idf.py build` 完整构建通过

### 5.2 功能验证清单（手动）
- [ ] 3个风扇均可独立控制（需硬件测试）
- [ ] MQTT命令可独立控制每个风扇（需硬件测试）
- [ ] MQTT状态上报包含3个风扇状态（需硬件测试）
- [ ] 本地模式下3个风扇同步动作（需硬件测试）
