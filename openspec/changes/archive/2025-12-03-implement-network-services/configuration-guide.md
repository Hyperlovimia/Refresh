...
由于和风天气的 `/air/now` 接口仅返回空气质量数据（PM2.5），不包含温度和风速，因此当前实现中：

- **PM2.5 数据**：从 API 实时获取 ✅
- **温度数据**：使用默认值 20.0°C（固定常量）⚠️
- **风速数据**：使用默认值 0.0 m/s（固定常量）⚠️

**影响范围**：
- 决策算法可以正常使用室外 PM2.5 数据判断是否开窗
- 如果决策算法需要真实室外温度/风速，需要额外调用 `/weather/now` 接口

**未来改进方向**：
1. 调用 `/weather/now` 接口获取温度和风速（需要额外 API 调用）
2. 使用组合接口一次性获取所有数据
3. 如果不需要温度/风速参与决策，可以保持现状
