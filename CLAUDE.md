...
在 ESP-IDF 实现模块时，优先用 exa MCP 查询官方文档。例如: ESP MQTT, ESP WiFi。
...
## 架构优先级
- "标准化 + 生态复用"拥有最高优先级，首先使用 exa MCP 查找并复用官方 SDK、社区成熟方案或既有模块。其次进行常规网络搜素，最后查询本地已有文件。
...
### 🧪 测试规范
- 每次实现建议提供可自动运行的单元测试或等效验证脚本，由本地 AI 执行。
...
### 💻 实现标准
- 除非用户明确要求，否则绝对禁止 MVP、最小实现或占位符；提交前必须完成全量功能与数据路径。
...
## 搜索工具优先级（必须）
- 外部信息检索优先使用 exa MCP；如需其他搜索引擎，需说明原因并补充引用。
